    ...
    try {
        ...
        for (const oldTable of OLD_TABLES) {
            ...
            try {
                ...
                const oldColumnsResult = await client.query(`SELECT column_name FROM information_schema.columns WHERE table_name = $1`, [oldTable]);
                const oldColumns = oldColumnsResult.rows.map(row => row.column_name);
                const newColumnsResult = await client.query(`SELECT column_name FROM information_schema.columns WHERE table_name = $1`, [newTable]);
