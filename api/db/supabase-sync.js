...
async function syncSchema() {
    ...
    `);
        const tableNames = existingTablesResult.rows.map(row => row.tablename);
...
async function migrateContentTables(tableNames) {
    ...
    `);
        const recordCount = parseInt(dataCheckResult.rows[0].count);
        if (recordCount > 0) {
...
async function setupTagRelationships(tableNames) {
    ...
    for (const tablePrefix of ['cms_']) {
      ...
      `);
            const existingTagsCount = parseInt(existingTagsCheck.rows[0].count);
    ...
    `);
        const oldTagsColumns = oldTagsColumnsResult.rows.map(row => row.column_name);
...
async function dropOldTables(tableNames) {
    if (!MIGRATE_AND_DROP) {
