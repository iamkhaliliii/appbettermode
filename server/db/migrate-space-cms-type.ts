...
async function migrateCmsTypeColumn() {
  ...
  try {
    ...
    const allSpaces = await db.select().from(schema.spaces) as Space[];
    ...
    const allCmsTypes = await db.select().from(schema.cms_types) as CmsType[];
    ...
    const cmsTypeById = new Map<string, CmsType>();
    const cmsTypeByName = new Map<string, CmsType>();
    
    allCmsTypes.forEach((cmsType: CmsType) => {
    ...
    for (const space of allSpaces) {
      const currentCmsType = space.cms_type as string;
      ...
      let targetCmsTypeId: string | null = null;
