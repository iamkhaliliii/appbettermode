...
/**
 * Migration script to:
 * 1. Add other_properties jsonb column to posts table
...
async function migrateDatabase() {
  ...
  try {
    ...
    const cmsTablesResult = await db<TableRow[]>`
      SELECT table_name 
    ...
    for (const table of cmsTables) {
      ...
      try {
        ...
        const hasPostIdResult = await db<ExistsResult[]>`
          SELECT EXISTS (
            SELECT 1 
        ...
        if (hasPostId) {
          ...
          const columnsResult = await db<ColumnRow[]>`
            SELECT column_name 
            FROM information_schema.columns 
            WHERE table_name = ${table} 
            AND column_name != 'post_id' 
