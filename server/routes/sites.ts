...
import { sites, memberships, spaces, cms_types, users } from '../db/schema.js';
...
router.get('/:identifier', async (req, res) => {
  try {
    ...
    let populatedContentTypes: Partial<Pick<typeof cms_types.$inferSelect, 'id' | 'name' | 'label' | 'icon_name' | 'color' | 'type' | 'fields'>>[] = [];
    const siteContentTypes = siteData.content_types as string[] | null; // Explicitly type
    ...
    return res.status(500).json({ message: 'Error fetching site from database', details: (error as Error).message });
  }
});
...
router.post('/', async (req, res) => {
  try {
    ...
    const siteValues: typeof sites.$inferInsert = {
      name: payload.name,
    ...
    if (contentTypeIds.length > 0) {
      ...
      for (const cmsType of cmsTypeDetails) {
        ...
        const spaceData: typeof spaces.$inferInsert = {
          name: spaceName,      
          slug: spaceSlug,      
      ...
      if (createdSpaceIds.length > 0) {
        ...
        (newDbSite as any).space_ids = createdSpaceIds; 
    ...
    const responseSite = {
      ...
      space_ids: (newDbSite as any).space_ids || [] 
    };
...
router.get('/:siteId/spaces', async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: 'Failed to fetch spaces', details: (error as Error).message });
  }
});
