...
import { posts, post_tags, tags, users, spaces, sites, cms_types } from '../db/schema.js';
import { eq, and, or, inArray, desc, SQL, sql } from 'drizzle-orm';
import { z } from 'zod';
...
router.get('/debug/discussions', async (req, res) => {
  try {
    ...
    const postsWithSpaceInfo = await Promise.all(
...
router.get('/mock/:siteIdentifier', async (req, res) => {
  try {
    ...
    for (const space of siteSpaces) {
      ...
      const templates = mockTemplates[cmsType.name as keyof typeof mockTemplates];
    ...
    const result = await db.transaction(async (tx) => {
      ...
      if (payload.tags && payload.tags.length > 0 && postResult.length > 0) {
        ...
        const existingTags = await tx
          ...
          .from(tags)
          .where(and(
            inArray(tags.id, payload.tags),
            eq(tags.site_id, payload.site_id)
          ));
...
router.put('/:postId', async (req, res) => {
  try {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      if (payload.published_at !== undefined) updateData.published_at = new Date(payload.published_at);
      if (payload.locked !== undefined) updateData.locked = payload.locked;
      ...
      if (payload.tags !== undefined) {
        ...
        if (payload.tags.length > 0) {
          ...
          const existingTags = await tx
            ...
            .where(and(
              inArray(tags.id, payload.tags),
              eq(tags.site_id, existingPost.site_id)
            ));
