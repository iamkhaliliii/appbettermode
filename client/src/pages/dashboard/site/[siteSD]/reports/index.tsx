...
export default function DashboardSiteReports() {
  const [location, setLocation] = useLocation();
  const [, params] = useRoute('/dashboard/site/:siteSD/reports/:section?');
  ...
  // Redirect to the first tab if we're at the root reports route
  useEffect(() => {
    if (siteDetails && location === APP_ROUTES.DASHBOARD_SITE.REPORTS(siteSD)) {
      setLocation(APP_ROUTES.DASHBOARD_SITE.REPORTS_SECTION(siteSD, 'overview'));
    }
  ...
  // If we're at the root reports URL and not yet redirected
  if (location === APP_ROUTES.DASHBOARD_SITE.REPORTS(siteSD)) {
    return (
      <DashboardLayout currentSiteIdentifier={siteSD} siteName={siteDetails.name}>
        <div className="flex items-center justify-center h-64">
          <Loader2 className="h-12 w-12 animate-spin text-primary-500" />
          <p className="ml-3 text-lg">Redirecting to Reports...</p>
        </div>
      </DashboardLayout>
    );
  }
      ...
      <div className="max-w-7xl mx-auto p-4">
        <div className="mb-8">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            View analytics and reports for {siteDetails.name}
          </p>
        </div>

        <Card>
          <CardHeader>
            <CardTitle>Reports - {section ? section.charAt(0).toUpperCase() + section.slice(1) : 'Overview'}</CardTitle>
          </CardHeader>
          <CardContent>
