...
export default function DashboardSiteAppearance() {
  const [location, setLocation] = useLocation();
  const [, params] = useRoute('/dashboard/site/:siteSD/appearance/:section?');
  ...
  // Redirect to the first tab if we're at the root appearance route
  useEffect(() => {
    if (siteDetails && location === APP_ROUTES.DASHBOARD_SITE.APPEARANCE(siteSD)) {
      setLocation(APP_ROUTES.DASHBOARD_SITE.APPEARANCE_SECTION(siteSD, 'theme'));
    }
  ...
  // If we're at the root appearance URL and not yet redirected
  if (location === APP_ROUTES.DASHBOARD_SITE.APPEARANCE(siteSD)) {
    return (
      <DashboardLayout currentSiteIdentifier={siteSD} siteName={siteDetails.name}>
        <div className="flex items-center justify-center h-64">
          <Loader2 className="h-12 w-12 animate-spin text-primary-500" />
          <p className="ml-3 text-lg">Redirecting to Appearance Settings...</p>
        </div>
      </DashboardLayout>
    );
  }
      ...
      <div className="max-w-7xl mx-auto p-4">
        <div className="mb-8">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Customize the look and feel of {siteDetails.name}
          </p>
        </div>

        <Card>
          <CardHeader>
            <CardTitle>Appearance - {section ? section.charAt(0).toUpperCase() + section.slice(1) : 'Theme'}</CardTitle>
          </CardHeader>
          <CardContent>
