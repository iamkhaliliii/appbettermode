...
export default function DashboardSiteAppStore() {
  ...
  // Redirect to the first tab if we're at the root app-store route
  useEffect(() => {
    if (siteDetails && location === APP_ROUTES.DASHBOARD_SITE.APP_STORE(siteSD)) {
  ...
  // If we're at the root app-store URL and not yet redirected
  if (location === APP_ROUTES.DASHBOARD_SITE.APP_STORE(siteSD)) {
    return (
      <DashboardLayout currentSiteIdentifier={siteSD} siteName={siteDetails.name}>
        <div className="flex items-center justify-center h-64">
          <Loader2 className="h-12 w-12 animate-spin text-primary-500" />
          <p className="ml-3 text-lg">Redirecting to App Store...</p>
        </div>
      </DashboardLayout>
    );
  }
      ...
      <div className="max-w-7xl mx-auto p-4">
        <div className="mb-8">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Browse and manage integrations and add-ons for {siteDetails.name}
          </p>
        </div>

        <Card>
          <CardHeader>
            <CardTitle>App Store - {section ? section.charAt(0).toUpperCase() + section.slice(1) : 'Browse'}</CardTitle>
          </CardHeader>
          <CardContent>
