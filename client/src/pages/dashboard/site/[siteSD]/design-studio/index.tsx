...
export default function DashboardSiteDesignStudio() {
  const [location, setLocation] = useLocation();
  const [, params] = useRoute('/dashboard/site/:siteSD/design-studio/:section?');
  ...
  // Redirect to the first tab if we're at the root design-studio route
  useEffect(() => {
    if (siteDetails && location === APP_ROUTES.DASHBOARD_SITE.DESIGN_STUDIO(siteSD)) {
      setLocation(APP_ROUTES.DASHBOARD_SITE.DESIGN_STUDIO_SECTION(siteSD, 'templates'));
    }
  ...
  // If we're at the root design-studio URL and not yet redirected
  if (location === APP_ROUTES.DASHBOARD_SITE.DESIGN_STUDIO(siteSD)) {
    return (
      <DashboardLayout currentSiteIdentifier={siteSD} siteName={siteDetails.name}>
        <div className="flex items-center justify-center h-64">
          <Loader2 className="h-12 w-12 animate-spin text-primary-500" />
          <p className="ml-3 text-lg">Redirecting to Design Studio...</p>
        </div>
      </DashboardLayout>
    );
  }
      ...
      <div className="max-w-7xl mx-auto p-4">
        <div className="mb-8">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Create and customize {siteDetails.name}'s design
          </p>
        </div>

        <Card>
          <CardHeader>
            <CardTitle>Design Studio - {section ? section.charAt(0).toUpperCase() + section.slice(1) : 'Templates'}</CardTitle>
          </CardHeader>
          <CardContent>
