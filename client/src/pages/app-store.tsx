...
export default function AppStore() {
  ...
  const [, params] = useRoute('/app-store/:section');
  const section = params?.section;

  // Redirect to the first tab (integrations) if we're at the root app-store route
  useEffect(() => {
    if (location === '/app-store') {
      setLocation('/app-store/integrations');
    }
      ...
      <div className="max-w-7xl mx-auto p-4">
        <div className="mb-8">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Browse and manage integrations and add-ons
          </p>
        </div>

        <div className="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6 border border-gray-200 dark:border-gray-700">
          <div className="text-center py-12">
            <h2 className="text-lg font-medium text-gray-900 dark:text-white mb-2">App Store</h2>
