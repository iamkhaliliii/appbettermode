        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-start justify-between">
            ...
            <div className="flex-shrink-0 ml-2 text-[10px] text-gray-500 dark:text-gray-400 whitespace-nowrap">
              ...
              {!notification.read && (
                <span className="ml-1 h-1.5 w-1.5 rounded-full bg-blue-500 inline-block"></span>
              )}
            </div>
          </div>
...
export function NotificationDrawer({
  ...
  const [activeSpaceFilter, setActiveSpaceFilter] = useState<string | null>(
    null,
  );
  ...
  const [activeFilterCategory, setActiveFilterCategory] =
    useState<FilterCategory>("status");
  ...
  const filteredNotifications = useMemo(() => {
    ...
    return notifications.filter((notification) => {
      ...
      // Apply space filter
      if (activeSpaceFilter && notification.space !== activeSpaceFilter)
        return false;

      // Apply CMS filter
      if (activeCmsFilter && notification.cmsType !== activeCmsFilter)
        return false;

      // Apply time filter
      if (activeTimeFilter && notification.timeCategory !== activeTimeFilter)
        return false;
  ...
  const groupedByTime = useMemo(() => {
    ...
    return Object.fromEntries(
      Object.entries(timeGroups).filter(
        ([_, notifications]) => notifications.length > 0,
      ),
    );
    ...
    if (activeTypeFilter) {
      filters.push(
        ...
        >
          ...
          <span className="mr-1">
            {activeTypeFilter.charAt(0).toUpperCase() +
              activeTypeFilter.slice(1)}
          </span>
          <span
            className="cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-600 rounded-full h-3.5 w-3.5 inline-flex items-center justify-center"
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange} direction="right">
      <DrawerContent className="right-0 left-auto w-full sm:w-[400px] max-w-full border-l border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 h-full">
                      ...
                      <DropdownMenuItem
                        ...
                        onClick={() => {
                          setActiveSpaceFilter(
                            activeSpaceFilter === space ? null : space,
                          );
                          setShowFilterDropdown(false);
                        }}
                      >
                  ...
                  >
                    {availableCmsTypes.map((cmsType) => (
                      <DropdownMenuItem
                        ...
                        onClick={() => {
                          setActiveCmsFilter(
                            activeCmsFilter === cmsType ? null : cmsType,
                          );
                          setShowFilterDropdown(false);
                        }}
                      >
                  ...
                  >
                    {Object.entries(timePeriods).map(([value, label]) => (
                      <DropdownMenuItem
                        ...
                        onClick={() => {
                          setActiveTimeFilter(
                            activeTimeFilter === value ? null : value,
                          );
                          setShowFilterDropdown(false);
                        }}
                      >
