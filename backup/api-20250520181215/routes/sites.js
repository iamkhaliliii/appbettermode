...
router.post('/:siteId/spaces', async (req, res) => {
    try {
        ...
        // Insert the new space with only basic columns
        const query = sql `
      INSERT INTO spaces 
      (name, slug, description, creator_id, site_id, visibility, cms_type, hidden)
      VALUES 
    ...
    `;
        const result = await db.execute(query);
        // Format the response
        const space = Array.isArray(result) && result.length > 0 ? result[0] : null;
        logger.info(`Space created successfully:`, space);
        if (!space) {
            return res.status(500).json({ message: 'Failed to create space - no result returned' });
        }
